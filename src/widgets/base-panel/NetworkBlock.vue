...
import type { Hardware } from '@widget-js/core'
...
function update(network: Hardware[]) {
  let uploadTotalSpeed = 0
  let downloadTotalSpeed = 0
  let downloadedDataTotal = 0
  let uploadedDataTotal = 0

  const throughput = network.map(it => it.children).flat().filter(it => it.sensorType == 'Throughput').map(it => it.children).flat()
  const networkData = network.map(it => it.children).flat().filter(it => it.sensorType == 'Data').map(it => it.children).flat()
...
<template>
  <div class="flex flex-col flex-1 justify-between h-full items-center network">
    <div>Net</div>
    <div class="data-label text-sm">
      ↓: {{ BytesUtils.pretty(downloadSpeed) }}
    </div>
    <div class="data">
      {{ downloadedData }}
    </div>
    <div class="data-label text-sm">
      ↑: {{ BytesUtils.pretty(uploadSpeed) }}
    </div>
    <div class="data">
      {{ uploadedData }}
    </div>
